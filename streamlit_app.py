...
TAB_NAMES = ["📊 실시간", "📅 일자별", "🎬 크리에이티브", "🔬 A/B", "🆕 신규소재", "🔁 중복DB", "📺 유튜브", "🔍 경쟁사", "📊 코호트", "🎯 KPI"]
...
# ============================================================
    ...
    if not cmp_mode:
        ...
        with st.expander("📋 일자별 상세 표 (선택 지표)"):
            piv = day.pivot_table(index="date", columns="creative", values=_dcol, aggfunc="first").sort_index(ascending=False)
            piv.index = piv.index.strftime("%-m월 %-d일")
