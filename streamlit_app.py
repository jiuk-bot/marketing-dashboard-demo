...
def gen():
    ...
    for ch, camp, ag, cr in structure:
        is_h = (cr == "영상소재 C")  # 가로형 장초수: CTR↑·CVR↓ (포맷 특성)
...
def _render_demo_chatbot():
    ...
    with st.container(key="dcmo"):
        ...
        with h1:
            st.markdown("**🗂 히스토리 챗봇**")
        with h2:
            if (not _min) and st.button("📝" if _mode == "chat" else "💬",
                                        key="dcmo_mode_b", help="저장/조회 전환"):
                st.session_state["dcmo_mode"] = "save" if _mode == "chat" else "chat"
                st.rerun(scope="fragment")
        with h3:
            if (not _min) and st.button("⛶", key="dcmo_full_b", help="전체창"):
                st.session_state["dcmo_full"] = not _full
                st.rerun(scope="fragment")
        with h4:
            if st.button("➕" if _min else "—", key="dcmo_tg", help="펼치기/최소화"):
                st.session_state["dcmo_min"] = not _min
                st.rerun(scope="fragment")
...
TAB_NAMES = ["📊 실시간", "📅 일자별", "🎬 크리에이티브", "🔬 A/B테스트", "🆕 신규소재", "🔁 월간 중복DB 현황", "📺 노출 Youtube 채널", "🔍 경쟁사 분석", "📊 코호트", "🎯 KPI"]
...
# ============================================================
    ...
    st.caption("영상 소재 중심 — 노출·도달·조회·CTR·CVR·CPA·CPM·CPC 등 다양한 지표 + 포맷 비교 + 피로도.")

    FMT = {"영상소재 A": "세로형 릴스", "영상소재 B": "세로형 15초", "영상소재 C": "가로형 장초수"}
    ...
    cch["포맷"] = cch.creative.map(FMT).fillna("기타")
    ...
    for col, cr in zip(pcols, ranked):
        fmt = FMT.get(cr, ""); val = float(ser.get(cr, 0)) if cr in ser.index else 0.0
        is_best = (cr == best_cr)
        ...
        col.markdown(
            ...
            f'<div style="position:absolute;bottom:8px;left:10px;color:#e6e9ee;font-size:12px;"><b>{cr}</b> · {fmt}</div>'
            f'</div>',
            unsafe_allow_html=True)
    ...
    if not cmp_mode:
        ...
        allc = [c for c in full.columns if c not in ("소재", "포맷")]
        sel = st.multiselect("표시 지표 (비우면 전체)", allc,
                             default=["노출", "조회수", "CTR(%)", "CPC", "CPM", "전환", "CVR(%)", "CPA", "ROAS(%)"], key="cr_cols")
        show = ["소재", "포맷"] + (sel if sel else allc)
        st.dataframe(full[show], use_container_width=True, hide_index=True)

        # ② 포맷별 비교 — 가로 vs 세로
        st.markdown("##### 포맷별 비교 — 가로형 vs 세로형")
        cd["watch"] = cd.creative.map(WATCH)
        fg = metrics(cd.groupby("포맷", as_index=False)[["spend", "impressions", "clicks", "conversions", "revenue"]].sum())
        fw = {fmt: round(float(np.average(gg.watch, weights=gg.impressions)), 1) for fmt, gg in cd.groupby("포맷")}
        k = st.columns(len(fg))
        for i, (_, r) in enumerate(fg.iterrows()):
            k[i].metric(r["포맷"], f"CVR {r.CVR:.2f}%", f"CTR {r.CTR:.2f}% · CPM {won(r.CPM)} · 평균시청 {fw.get(r['포맷'], 0)}s", delta_color="off")
        g1, g2 = st.columns(2)
        with g1:
            st.markdown("**포맷별 CVR(%)**")
            f = go.Figure(); f.add_bar(x=fg["포맷"], y=fg.CVR, marker_color=ACCENT, text=[f"{v:.2f}%" for v in fg.CVR], textposition="outside", cliponaxis=False)
            f.update_yaxes(range=[0, max(fg.CVR.max(), 0.1) * 1.25])
            st.plotly_chart(chart(f, 280), use_container_width=True)
        with g2:
            st.markdown("**포맷별 CTR(%)**")
            f = go.Figure(); f.add_bar(x=fg["포맷"], y=fg.CTR, marker_color=AMBER, text=[f"{v:.2f}%" for v in fg.CTR], textposition="outside", cliponaxis=False)
            f.update_yaxes(range=[0, max(fg.CTR.max(), 0.1) * 1.25])
            st.plotly_chart(chart(f, 280), use_container_width=True)
        st.caption("💡 가로형은 CTR↑·평균 시청 시간↑(장초수)지만 CVR↓ → 클릭/조회만 보면 가로형을 키우는 오판. CVR·CPA로 봐야 세로형 우위.")

        # ③ 소재 피로도 — CTR 추이
        st.markdown("##### 소재별 CTR 추이 (피로도 감지)")
        ts = cd.groupby(["date", "creative"], as_index=False)[["clicks", "impressions"]].sum()
        ts["CTR"] = (ts.clicks / ts.impressions * 100).round(3)
        f = go.Figure()
        for cr in cr_list:
            s = ts[ts.creative == cr]
            f.add_trace(go.Scatter(x=s.date, y=s.CTR, name=f"{cr}({FMT.get(cr,'')})", mode="lines"))
        f.update_xaxes(tickformat="%-m월 %-d일")
        st.plotly_chart(chart(f, 320), use_container_width=True)
        st.caption("CTR이 추세적으로 하락하면 소재 피로도 신호 → 로테이션/교체 검토. (단일 일자 변동은 노이즈)")
    else:
        def _m(frame):
        ...
        for cr in cr_list:
            ...
            rows.append({"소재": cr, "포맷": FMT.get(cr, ""),
        ...
        st.caption("전환 변화 ↑ / CPA 변화 ↓ 이면 개선. 어떤 소재가 기간 사이 좋아졌는지/나빠졌는지 비교.")

        st.markdown("##### 포맷별 CVR — 기간 A vs B")
        def _fmtcvr(frame):
            out = {}
            for fmt, g in frame.groupby("포맷"):
                ck = g.clicks.sum(); out[fmt] = round(g.conversions.sum() / ck * 100, 2) if ck else 0
            return out
        fa = _fmtcvr(A); fb = _fmtcvr(B); fmts = sorted(set(fa) | set(fb))
        ff = go.Figure()
        ff.add_bar(x=fmts, y=[fa.get(m, 0) for m in fmts], name="기간 A", marker_color=AMBER)
        ff.add_bar(x=fmts, y=[fb.get(m, 0) for m in fmts], name="기간 B", marker_color=ACCENT)
        st.plotly_chart(chart(ff, 300), use_container_width=True)
...
# ============================================================
    ...
    _FMTN = {"영상소재 A": "세로형 릴스", "영상소재 B": "세로형 15초", "영상소재 C": "가로형 장초수"}
